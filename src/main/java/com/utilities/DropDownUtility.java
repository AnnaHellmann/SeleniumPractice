...
public class DropDownUtility extends Utility {
    ...
    public static List<String> getAllSelectedOptions(By locator){
        List<WebElement> allSelectedElements = findDropDown(locator).getAllSelectedOptions();
        return allSelectedElements.stream().map(WebElement::getText).collect(Collectors.toList());
    }
}
